...
export class Server {
  ...
  static async usingAsync(handlerAsync: (context: app.Context) => Promise<void>) {
    const server = await this.createAsync();
    await handlerAsync(new app.Context(app.settings.serverUrl)).finally(() => server.disposeAsync());
  }

...
export class RemoteController {
  ...
  constructor(cacheService: app.CacheService, contextService: app.ContextService) {
  ...
  async popularAsync(@api.Query() model: app.api.RemoteQueryPopular) {
    const cacheKey = `remote/popular/${model.providerName}/${model.pageNumber || 1}`;
    const cacheTimeout = app.settings.cacheRemoteSearchTimeout;
    return await this._cacheService.getAsync(cacheKey, cacheTimeout, async () => {
      const context = this._contextService.get();
      return await app.provider.popularAsync(context, model.providerName, model.pageNumber);
    });
  }
  ...
  async seriesAsync(@api.Query() model: app.api.RemoteQuerySeries) {
    ...
    return await this._cacheService.getAsync(cacheKey, cacheTimeout, async () => {
      const context = this._contextService.get();
      return await app.provider.seriesAsync(context, model.url);
    });
  }
  ...
  async streamAsync(@api.Query() model: app.api.RemoteQueryStream) {
    ...
    return await this._cacheService.getAsync(cacheKey, cacheTimeout, async () => {
      const context = this._contextService.get();
      return await app.provider.streamAsync(context, model.url);
    });
  }
}
